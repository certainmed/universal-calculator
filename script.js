...
function generateCalculatorCards(category, subcategory) {
  ...
  calculatorList.forEach((calc) => {
    const titleId = `${calc.id}-title`;
    const descId = `${calc.id}-desc`;
    cardsHTML += `
                    <div class="calculator-card" data-calculator="${calc.id}" tabindex="0" role="button" aria-labelledby="${titleId}" aria-describedby="${descId}">
  ...
  container.querySelectorAll(".calculator-card").forEach((card) => {
    ...
    const openCalculator = () => showCalculator(category, subcategory, calculatorId);

    card.addEventListener("click", openCalculator);
